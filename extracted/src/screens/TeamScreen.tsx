...
import { TEAM_TEXTS, translateCropSpecialty, translateLanguages } from './teamTranslations';
...
export default function TeamScreen() {
  ...
  const ln = TEAM_TEXTS[language] || TEAM_TEXTS.en;

  const tRole = (role: string) => ln.roles[role] || role;
  const tSubrole = (subrole?: string) => {
    if (!subrole) return undefined;
    return ln.subRoles[subrole] || subrole;
  };
  const tSpecialty = (spec?: string) => {
    return translateCropSpecialty(spec || "", language);
  };
  ...
  const filteredTeam = useMemo(() => {
    return teamMembers.filter(m => {
      ...
      const roleText = tRole(m.role).toLowerCase();
      const matchesRole = m.role.toLowerCase().includes(cleanSearch) || roleText.includes(cleanSearch);
      
      const specialtyText = m.specialty ? tSpecialty(m.specialty).toLowerCase() : '';
      const matchesSpecialty = (m.specialty?.toLowerCase().includes(cleanSearch) || false) || specialtyText.includes(cleanSearch);
      
      const subroleText = m.subRole ? (tSubrole(m.subRole) || "").toLowerCase() : '';
      const matchesSubrole = (m.subRole?.toLowerCase().includes(cleanSearch) || false) || subroleText.includes(cleanSearch);

      return matchesDept && (matchesName || matchesRole || matchesSpecialty || matchesSubrole);
    });
  }, [searchTerm, selectedDept, teamMembers, language]);

  const departmentsConfig = [
    { id: 'all', name: ln.departments.all?.name || 'All Members', label: ln.departments.all?.label || 'All', color: 'border-emerald-600' },
    { id: 'management', name: ln.departments.management?.name, label: ln.departments.management?.label, color: 'border-amber-500' },
    { id: 'sales', name: ln.departments.sales?.name, label: ln.departments.sales?.label, color: 'border-teal-500' },
    { id: 'operations', name: ln.departments.operations?.name, label: ln.departments.operations?.label, color: 'border-blue-500' },
    { id: 'technical', name: ln.departments.technical?.name, label: ln.departments.technical?.label, color: 'border-indigo-500' },
    { id: 'service', name: ln.departments.service?.name, label: ln.departments.service?.label, color: 'border-purple-500' },
    { id: 'suppliers', name: ln.departments.suppliers?.name, label: ln.departments.suppliers?.label, color: 'border-emerald-500' }
  ];

  // Ordered sections corresponding to original design for beautiful modern sectionized layout
  const orderedSections = [
    { id: 'management', name: ln.departments.management?.name.toUpperCase(), subtitle: ln.departments.management?.subtitle, desc: ln.departments.management?.desc },
    { id: 'sales', name: ln.departments.sales?.name.toUpperCase(), subtitle: ln.departments.sales?.subtitle, desc: ln.departments.sales?.desc },
    { id: 'operations', name: ln.departments.operations?.name.toUpperCase(), subtitle: ln.departments.operations?.subtitle, desc: ln.departments.operations?.desc },
    { id: 'technical', name: ln.departments.technical?.name.toUpperCase(), subtitle: ln.departments.technical?.subtitle, desc: ln.departments.technical?.desc },
    { id: 'service', name: ln.departments.service?.name.toUpperCase(), subtitle: ln.departments.service?.subtitle, desc: ln.departments.service?.desc },
    { id: 'suppliers', name: ln.departments.suppliers?.name.toUpperCase(), subtitle: ln.departments.suppliers?.subtitle, desc: ln.departments.suppliers?.desc }
  ];
      ...
      >
        ...
        <div className="w-full aspect-[3/4] sm:aspect-[4/5] relative overflow-hidden bg-slate-50 flex-shrink-0">
          ...
          <span className="absolute top-4 left-4 text-[9px] uppercase tracking-widest font-mono font-black px-2.5 py-1.5 bg-[#01261d]/85 text-amber-400 rounded-xl backdrop-blur-md border border-emerald-800/30 shadow-md">
            {ln.departments[member.department]?.label || member.department}
          </span>
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white z-10 block lg:hidden group-hover:block transition-all duration-500">
            <span className="text-[10px] font-mono tracking-wider text-amber-400 font-bold uppercase transition-transform">{ln.clickToViewBio}</span>
          </div>
        </div>
        ...
        <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
          <div className="space-y-1.5">
            ...
            {member.subRole && (
              <p className="text-slate-500 text-xs leading-relaxed line-clamp-3 text-justify font-sans pt-1">
                {tSubrole(member.subRole)}
              </p>
            )}

            {member.specialty && (
              <div className="pt-2">
                <span className="text-[9px] tracking-wider text-slate-400 font-black uppercase block mb-1">{ln.cropsSpecialtiesLabel}</span>
          ...
          {member.languages && member.languages.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {translateLanguages(member.languages, language).map(lang => (
                <span 
                  key={lang} 
    ...
    <div className="bg-[#fbfcfa] min-h-screen pt-24 sm:pt-28 md:pt-[148px] pb-24 text-slate-800 font-sans" id="team-screen-view">
      ...
      <div className="bg-[#f2f4f1] border-y border-slate-200/60 py-4 mb-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between text-xs text-slate-500">
          <span className="font-bold uppercase tracking-wider text-[#01261d] font-sans">
            {language === 'fa' ? 'درباره ما' : language === 'ru' ? 'О нас' : language === 'ar' ? 'معلومات عنا' : 'About Us'}
          </span>
          <div className="flex items-center gap-1.5 text-[11px]">
            <a href="#/" className="hover:text-emerald-800 transition-colors">
              {language === 'fa' ? 'خانه' : language === 'ru' ? 'Главная' : language === 'ar' ? 'الرئيسية' : 'Home'}
            </a>
            <span className="text-slate-400">/</span>
            <span className="font-medium text-slate-400">
              {language === 'fa' ? 'تیم متخصصان' : language === 'ru' ? 'Наша команда' : language === 'ar' ? 'فريق العمل' : 'Expert Team'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
          ...
          <div className="lg:col-span-6 space-y-6 text-left relative z-10">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 px-4 py-2 rounded-2xl">
                ...
                <span className="text-xs font-bold text-slate-700">{ln.statStaff}</span>
              </div>
              <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 px-4 py-2 rounded-2xl">
                <Globe className="w-4 h-4 text-amber-500" />
                <span className="text-xs font-bold text-slate-700">{ln.statLang}</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 w-full relative z-10">
            <div className="grid grid-cols-3 gap-2 p-2 rounded-2xl bg-slate-100/80 border border-slate-205/60 relative">
              ...
              >
                ...
                <span className="text-xs uppercase tracking-wider">{ln.watchVideoBtn}</span>
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
                ...
                <div className="w-72 sm:w-80 h-96 rounded-[24px] overflow-hidden bg-emerald-950 border-4 border-amber-500 p-2 shadow-2xl relative">
                  ...
                  <div className="absolute top-4 left-4 bg-[#01261d]/90 border border-emerald-800 backdrop-blur-md px-3 py-1 rounded-xl flex items-center gap-1.5 shadow">
                    ...
                    <span className="text-[10px] font-mono tracking-widest text-amber-400 font-bold uppercase">
                      {language === 'fa' ? 'مدیر عامل گیلافروت' : language === 'ru' ? 'CEO GilaFruit' : language === 'ar' ? 'الرئيس التنفيذي' : 'GilaFruit CEO'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-7 space-y-6 text-left">
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-white leading-none font-display">
                {ln.spotlightTitle.includes(':') ? (
                  <>
                    {ln.spotlightTitle.split(':')[0]}: <span className="text-amber-400">{ln.spotlightTitle.split(':')[1]}</span>
                  </>
                ) : ln.spotlightTitle.includes('：') ? (
                  <>
                    {ln.spotlightTitle.split('：')[0]}: <span className="text-amber-400">{ln.spotlightTitle.split('：')[1]}</span>
              ...
              <div className="pt-6 border-t border-emerald-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                  <span className="block text-[10px] text-emerald-400 font-mono uppercase tracking-widest leading-none mb-1">{ln.spotlightSignatureLabel}</span>
                  <div className="font-serif italic text-2xl font-light text-amber-300 font-sans tracking-wide">
                    {ln.spotlightTitle.includes(':') ? ln.spotlightTitle.split(':').pop()?.trim() : ln.spotlightTitle.includes('：') ? ln.spotlightTitle.split('：').pop()?.trim() : ln.spotlightTitle}
                  </div>
                </div>
        ...
        <div className="space-y-8">
          ...
          <div className="text-left space-y-4 max-w-3xl">
            <h2 className="text-3xl sm:text-4xl font-black tracking-tight text-[#01261d]-950 font-display">
              {ln.directoryTitle}
            </h2>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/60 p-4 shadow-sm grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
            ...
            <div className="md:col-span-12 lg:col-span-7 flex flex-wrap items-center justify-start lg:justify-end gap-2 text-xs">
          ...
          <div className="flex items-center justify-between text-xs text-slate-400 font-mono">
            <span>{ln.showingStats.replace('{filtered}', filteredTeam.length.toString()).replace('{total}', teamMembers.length.toString())}</span>
            {searchTerm && <span>{ln.searchResultFor} "{searchTerm}"</span>}
          </div>
            ...
            {filteredTeam.length > 0 ? (
              ...
              <div className="bg-slate-100 rounded-3xl border border-dashed border-slate-300 py-16 text-center text-slate-500 max-w-xl mx-auto space-y-3 mt-8">
                ...
                <h3 className="text-base font-black text-slate-800">
                  {language === 'fa' ? 'هیچ عضوی یافت نشد' : language === 'ru' ? 'Совпадений не найдено' : 'No matching members found'}
                </h3>
                <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">
                  {language === 'fa' 
                    ? 'هیچ کاربری با معیارهای جستجوی شما مطابقت ندارد. عبارت دیگری را امتحان کنید.' 
                    : language === 'ru' 
                      ? 'Мы не смогли найти сотрудников по вашему запросу. Попробуйте ввести другие ключевые слова.' 
                ...
                >
                  {language === 'fa' ? 'بازنشانی تفکیک‌ها' : language === 'ru' ? 'Сбросить фильтры' : 'Reset all filters'}
                </button>
              </div>
            )}
          </AnimatePresence>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-8 space-y-4">
              <h3 className="text-2xl font-extrabold text-white tracking-tight">
                {language === 'fa' ? 'تعهد ما به همکاری صادقانه و مستمر' : language === 'ru' ? 'Наша гарантия ответственного сотрудничества' : 'Our Guarantee of Diligent Collaboration'}
              </h3>
              <p className="text-emerald-100/90 text-xs sm:text-sm leading-relaxed max-w-4xl text-justify">
                {language === 'fa' 
                  ? 'تمامی اعضای تیم در شرکت سبز گستران گیلافروت تحت اصول رفتار حرفه‌ای و تحویل به موقع بارها فعالیت دوجانبه دارند. کادر چندزبانه ما مرزهای زبانی را از بین می‌برد.' 
                  : language === 'ru' 
                    ? 'Каждый специалист Sabz Gostaran Gilan Fruit Co. работает строго в рамках взаимных соглашений по качеству и графиков поставок.' 
                    : 'Every team member at Sabz Gostaran Gilan Fruit Co. operates under a strict code of mutual customer support and premium delivery schedules.'}
              </p>
            </div>
            <div className="lg:col-span-4 flex justify-start lg:justify-end gap-3 font-mono text-xs text-emerald-200">
              <div className="flex items-center gap-1.5 bg-emerald-950/60 px-4 py-2.5 rounded-2xl border border-emerald-800/40">
                ...
                <span>{language === 'fa' ? 'پشتیبانی چندزبانه' : language === 'ru' ? 'Мультиязычная поддержка' : 'Multilingual Support'}</span>
              </div>
              <div className="flex items-center gap-1.5 bg-emerald-950/60 px-4 py-2.5 rounded-2xl border border-emerald-800/40">
                <Shield className="w-5 h-5 text-emerald-400" />
                <span>{language === 'fa' ? 'تضمین کیفیت ممتاز' : language === 'ru' ? 'Высокие стандарты качества' : 'Premium Quality Standards'}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            >
              ...
              <div className="relative aspect-video bg-black flex items-center justify-center group overflow-hidden">
                ...
                <div className="relative z-10 text-center space-y-4 max-w-md px-6">
                  ...
                  <p className="text-xs text-emerald-100 leading-relaxed text-justify">
                    {ln.videoDesc}
                  </p>
              ...
              <div className="p-4 bg-emerald-950/50 text-slate-400 text-center text-xs border-t border-emerald-900 flex justify-between items-center px-6">
                <span>{ln.videoDuration}</span>
                <span className="text-[#1a8a42] font-semibold text-[11px]">{ln.videoContrast}</span>
              </div>
            ...
            >
              ...
              <div className="p-8 space-y-6">
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-slate-400 font-mono">{ln.storyMeta}</p>
                </div>
                ...
                <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100 text-xs text-[#1a8a42] font-semibold leading-relaxed text-justify">
                  "{ln.storyQuote}"
                </div>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <span className="inline-block text-[9px] uppercase tracking-widest font-mono font-bold px-2 py-0.5 bg-slate-100 text-slate-600 rounded">
                      {language === 'fa' 
                        ? 'بخش: ' + (ln.departments[selectedMember.department]?.label || selectedMember.department) 
                        : language === 'ru' 
                          ? 'Отдел: ' + (ln.departments[selectedMember.department]?.label || selectedMember.department) 
                          : 'Div: ' + (ln.departments[selectedMember.department]?.label || selectedMember.department)}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3.5 border-t border-slate-100 pt-4">
                  ...
                  {selectedMember.subRole && (
                    <div className="space-y-1">
                      <span className="text-[10px] tracking-wider text-slate-400 font-bold uppercase block">{ln.coreMandatesLabel}</span>
                      <p className="text-xs text-slate-600 leading-relaxed font-sans text-justify">
                        {tSubrole(selectedMember.subRole)}
                      </p>
                    </div>
                  )}
                  ...
                  {selectedMember.specialty && (
                    <div className="space-y-1">
                      <span className="text-[10px] tracking-wider text-slate-400 font-bold uppercase block">{ln.assignedCropsLabel}</span>
                      <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100 text-xs font-bold text-[#1a8a42] leading-relaxed text-justify">
                        🌿 {tSpecialty(selectedMember.specialty)}
                      </div>
                    </div>
                  )}
                  ...
                  {selectedMember.languages && selectedMember.languages.length > 0 && (
                    <div className="space-y-1">
                      <span className="text-[10px] tracking-wider text-slate-400 font-bold uppercase block">{ln.spokenLanguagesLabel}</span>
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {translateLanguages(selectedMember.languages, language).map(lang => (
                          <span 
                            key={lang} 
                ...
                <div className="space-y-2 border-t border-slate-100 pt-4">
                  {selectedMember.whatsapp ? (
                    ...
                    >
                      <Phone className="w-4 h-4" />
                      <span>{ln.msgCentralLineBtn}</span>
                      <ExternalLink className="w-3.5 h-3.5 opacity-60" />
                    </a>
                  )}
