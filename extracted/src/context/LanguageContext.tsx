...
import { getLProduct as getLP, getLBlog as getLB } from '../utils/localizedData';
...
interface LanguageContextType {
  ...
  getLProduct: (product: Product) => Product;
  getLBlog: (blog: BlogArticle) => BlogArticle;
}
...
export function LanguageProvider({ children }: LanguageProviderProps) {
  ...
  // Sync state when URL popstate, hashchange, or search updates
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Dynamically set fonts from active locale JSON data
    if (activeLocale && activeLocale.font) {
      const primary = activeLocale.font.primary || "Inter";
      const secondary = activeLocale.font.secondary || "Roboto";
      const fallback = activeLocale.font.fallback || "sans-serif";
      
      const sansFont = `"${primary}", "${secondary}", ${fallback}`;
      const displayFont = `"${primary}", "${secondary}", ${fallback}`;
      
      document.documentElement.style.setProperty('--current-font-sans', sansFont);
      document.documentElement.style.setProperty('--current-font-display', displayFont);
    }
  }, [language, direction, activeLocale]);
  ...
  // Helper to normalize and clean Persian/Arabic variations for slug comparison
  const normalizeSlug = (str: string): string => {
    let decoded = str;
    try {
      decoded = decodeURIComponent(str);
    } catch (e) {
      // Ignore URL encoding errors and keep raw
    }
    return decoded
      .toLowerCase()
      .replace(/[\u064a\u0649]/g, '\u06cc') // normalize Arabic Ya to Persian Yeh (ی)
      .replace(/[\u0643]/g, '\u06a9')     // normalize Arabic Kaf to Persian Keheh (ک)
      .replace(/[\u200c]/g, '-')          // replace ZWNJ with hyphen
  ...
  const resolveActiveView = (): ActiveViewDetails => {
    let cleanPath = decodeURIComponent(currentPath).replace(/^\/+|\/+$/g, '').toLowerCase();

    // Support hash router fallback first: if window.location.hash has #/abc, extract abc
    const currentHash = window.location.hash;
    if (currentHash && currentHash.startsWith('#/')) {
      cleanPath = decodeURIComponent(currentHash.slice(2)).replace(/^\/+|\/+$/g, '').toLowerCase();
    }
    ...
    // Help with "?lang=xx" or similar trailing stuff if present in cleanPath
    const queryIdx = cleanPath.indexOf('?');
    if (queryIdx !== -1) {
      cleanPath = cleanPath.substring(0, queryIdx);
    }

    // Normalize comparison pathway
    const normCleanPath = normalizeSlug(cleanPath);

    // 1. Check if matches localized static page route
    for (const [key, val] of Object.entries(activeLocale.routes)) {
      if (
        normalizeSlug(val as string) === normCleanPath ||
        (val as string).toLowerCase() === cleanPath
      ) {
        return { view: key };
      }
    }
    ...
    if (cleanPath.includes('#')) {
      ...
      if (hashPart.startsWith('product/')) {
        const rawSlug = hashPart.replace('product/', '');
        return { view: 'product-detail', slug: getOriginalProductKey(rawSlug) };
      }
      if (hashPart.startsWith('blog/')) {
        const rawSlug = hashPart.replace('blog/', '');
        return { view: 'blog-detail', id: getOriginalBlogKey(rawSlug), slug: getOriginalBlogKey(rawSlug) };
      }
    ...
    if (viewName && typeof viewName === 'string') {
      if (viewName.startsWith('#/')) {
        ...
        if (cleanHash === '') {
          ...
          resolvedSlug = getOriginalProductKey(cleanHash.slice(8));
        } else if (cleanHash.startsWith('blog/')) {
          resolvedView = 'blog-detail';
          resolvedSlug = getOriginalBlogKey(cleanHash.slice(5));
        ...
        if (cleanPath === '') {
          ...
          resolvedView = 'product-detail';
          resolvedSlug = getOriginalProductKey(cleanPath.slice(8));
        } else if (cleanPath.startsWith('blog/')) {
          resolvedView = 'blog-detail';
          resolvedSlug = getOriginalBlogKey(cleanPath.slice(5));
        } else {
  ...
  const getLProduct = (product: Product): Product => {
    return getLP(product, language);
  };

  const getLBlog = (blog: BlogArticle): BlogArticle => {
    return getLB(blog, language);
  };
